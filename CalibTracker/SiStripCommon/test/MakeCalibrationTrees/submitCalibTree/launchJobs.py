...
def getDatasetFromPattern(pattern,conf):
   ...
   datasets = []
   for line in result:
      if checkDatasetStructure(line,silent=True):
...
def sendJob(dataset,run,files,conf,firstFile):
   cmd = "python %s/submitCalibTree/runJob.py -f %s --firstFile %s -d %s -r %s -o %s"%(conf.RUNDIR, files,firstFile,dataset,run, conf.OUTDIR)
   if conf.AAG:
      cmd+=" -a "
