...
parser.add_option('-o', '--out'        ,    dest='outDir'             , help='out directory'                            , default='')
...
if(int(exit_code)!=0):
   ...
   if(int(FileSizeInKBytes)>50): 
      ...
      stageOutCode&= not os.system(conf.initEnv+'cmsStageOut -f '+PWDDIR+'/'+outfile + ' ' + conf.CASTORDIR+'/'+outfile)
      print conf.eosLs + conf.CASTORDIR+'/'+outfile
      stageOutCode&= not os.system(conf.eosLs + conf.CASTORDIR+'/'+outfile)
   else:
