...
#PCLDATASET = '/StreamExpress/Run2016D-PromptCalibProdSiStripGains-Express-v*/ALCAPROMPT'
PCLDATASET = "/StreamExpress/Run2016*-PromptCalibProdSiStripGainsAfterAbortGap-Express-v*/ALCAPROMPT"
#PCLDATASET = '/StreamExpress/Run2016B-PromptCalibProdSiStripGainsAfterAbortGap-Express-v*/ALCAPROMPT'
CALIBTREEPATH = '/store/group/dpg_tracker_strip/comm_tracker/Strip/Calibration/calibrationtree/GR16_Aag' #used if usePCL==False
...
calMode  = str(opt.calMode) if not str(opt.calMode)=='' else "AagBunch"
MC=""
...
maxNEvents = 5000   # maximum events allowed in a gain payload computation
...
if(usePCL==True):
   ...
   datasets = [ line for line in commands.getstatusoutput(initEnv+
                                        "das_client  --limit=9999 --query='dataset=%s'"%PCLDATASET)[1].splitlines()
   ...
   for dataset in datasets:
       runs += [ (dataset, line) for line in commands.getstatusoutput(initEnv+
                                         "das_client  --limit=9999 --query='run dataset=%s'"%dataset)[1].splitlines()
                      if not line.startswith('Showing') and 'SCRAM fatal' not in line and len(line)>0 ]
   ...
   for dataset, run_number in runs:
      ...
      NEventsDasOut = [ line for line in commands.getstatusoutput(initEnv+
       "das_client  --limit=9999 --query='summary dataset=%s run=%i | grep summary.nevents'"%(dataset,run))[1].splitlines()
      ...
      resultsFiles = [ line for line in commands.getstatusoutput(initEnv+
                       "das_client  --limit=9999 --query='file dataset=%s run=%i'"%(dataset,run))[1].splitlines()
                       if not line.startswith('Showing') and 'SCRAM fatal' not in line and len(line)>0 ]
...
if(automatic==True and NTotalEvents<10):	#ask at least 1M events to perform the calibration
